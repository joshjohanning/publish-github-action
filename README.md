...
| `comment_on_linked_issues`  | Comment on closed issues linked to PRs in the release notes to notify followers of the release (uses GraphQL `closingIssuesReferences`; idempotent — updates existing comment on rerun). Also posts on draft releases so the comment is ready when the release is published; the comment is updated if the version changes on a subsequent run. | No       | `false`                 |
